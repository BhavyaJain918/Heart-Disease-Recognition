...
(dataset_train , dataset_valid) = keras.utils.image_dataset_from_directory("" , labels = "inferred" , label_mode = "categorical" , image_size = (512 , 512) , batch_size = 32 , validation_split = 0.20 , subset = "both" , seed = 42)   # Path to training directory
...
model.fit(x = dataset_train , validation_data = dataset_valid , batch_size = 32 , epochs = 8)
...
while True:
  ...
  if key == ord("q") or cv2.getWindowProperty("Frame" , cv2.WND_PROP_VISIBLE) < 1:
    ...
    cv2.imwrite("Saved_Image_New.jpg" , frame)
    image = load_img(os.getcwd() + "\\Saved_Image_New.jpg")
    predict(image)
  else:
    continue
